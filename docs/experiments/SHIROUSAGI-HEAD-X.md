...
- **命名、文件夹和 group 绑定缺口**：这份 PSD 导入后是 24 个平铺图层、零个 group。实验脚本按 `ArtMesh` 名称配对，并用硬编码的 `ArtMesh30` 排除身体，再逐层建立 23 个 binding。正常创作流程应先整理图层名称与 Head、Face、Ears、Body 等文件夹或 group 关系，在头部 group 上建立共同的 X 轴绑定，再对需要不同形变的图层细调。需要面向 PSD 作者的命名、分组和 group mesh binding 功能；不能把已有 Part 组织关系直接视为满足这个需求。
