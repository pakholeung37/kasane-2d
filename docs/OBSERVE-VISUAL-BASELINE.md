...
## Default Framework texture filtering baseline (2026-09-26)

The O0 measurements above remain the historical record. The accepted default
`framework-texture-filtering` feature now generates source texture mipmaps,
uses linear pixel and mip-level filtering with repeating source UVs, and keeps
one framebuffer sample. The local Cubism Native OpenGL sample uses the same
texture sampling settings. Mipmap texels are generated by different APIs, so
this is a configuration baseline rather than a claim of byte-identical output
between OpenGL and WGPU.

The following raw RGBA measurements used the unchanged fixture manifest
`a971cfc4a93a55c3a335ae35df8f425968f4e5490dbe3ce1c333a0eacf530331`
and a release GPU wheel with native binary SHA-256
`e58acc8f5e074bd557a89404e22859346b9ea708ca2f4617b4e8059924a2c18a`
on Apple M4 / Metal. The target was 128×128 transparent RGBA8Unorm; elapsed
times are descriptive medians of three in-process calls.

| Case | Raw SHA-256 | Nonzero alpha pixels | Partial alpha pixels | Median ms |
| --- | --- | ---: | ---: | ---: |
| pixel-neutral | `05cfb59df221b0a10e8d4e646d479e1c1ea52a894f26532c772a321dd39ca764` | 956 | 472 | 4.672 |
| pixel-quarter | `e56756182bb77f2b551384db84209c7a634c1660cb64528a385cf159d04d6fa6` | 957 | 451 | 1.678 |
| pixel-full | `6c1cdcf10765d33cd496ad9f4f8e0fd715b7f5a265ad15b76feb28cbc536ba95` | 957 | 451 | 1.608 |
| rotated-warp | `e0bb95958fbebe283972ad990f02a2444767cefef5754912d4d6663b7adae0c0` | 223 | 223 | 1.537 |
| masked-offscreen | `7a12c63496544a40b5a0295729bb59629a9b952dfad33ff581b33401e171c6b3` | 1932 | 696 | 2.024 |

The full record is `target/observe-visual-framework-baseline.json`. Reproduce it
with an installed release wheel built using `--features observe` and the runner
above, setting `--output` to that path. These hashes are local GPU evidence,
not portable golden values. Peak whole-process RSS in this run was 68,452,352
bytes; it is not isolated Observe memory. A release wheel built with
`--no-default-features --features observe` reproduced all five historical O0
raw hashes exactly on the same adapter. The accepted default changes four of
the five fixture images; `masked-offscreen` is unchanged.
