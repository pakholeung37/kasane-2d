...
## 当前 S6 验收证据（2026-09-21）

S6 的 9 项门禁已通过，完整记录见 [S6 实现与验收](S6-offscreen-composition-fix.md) 和 [统一报告](../target/kasane/m3c/s6_report.json)。验证环境为 macOS / Godot 4.7.2 Compatibility / Apple M4，正式源码 Editor 使用本次 debug GDExtension。

| 能力 | 已实现 | 构造 / 专项证据 | 真实模型证据 |
|---|---|---|---|
| v6 Offscreen 预览 | Core render plan、父子合成、遮罩、动态依赖、编辑持久化与再导出 | 106 项生命周期检查；目标复制负向对照 | Ren 24 层、198 Drawable、246 命令；默认、参数变化和编辑后导出由官方 Framework 对照，活动表面逐组 ROI 达标 |
| 扩展 Color/Alpha | 18 Color × 5 Alpha 合法编码；两个兼容模式遵循旧 Alpha 规则 | 官方 SDK GLSL 独立矩阵，2,160 个逐项图像样本通过 | Ren 覆盖素材实际使用的模式；不宣称真实素材涵盖所有组合 |
| 资源预算 | 预览分辨率、稳定帧复用、空/禁用组清理、512 MiB 附件前置预算 | 超限拒绝、恢复、重父级、相机 A→B→A | Ren 2048×2048 渲染资源采样峰值约 552 MiB，低于 768 MiB 门限；该数字为引擎统计而非驱动总显存 |
| 旧场景回归 | 旧三种混合、M4/M3B 路径保留 | M4 数据 151 项、GPU 84 项，M3B 3,505 组双 Core 数值 | Mao 正式源码 Editor 语义编辑、脱离源文件重开、再导出及官方图像对照通过 |

S6 通过不等于 M3C 全部版本和发布闭环完成；真实外部 4.2 样本与 S7 的剩余门禁仍以实施计划记录为准。
