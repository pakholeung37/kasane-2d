...
func run():
    ...
    check(doc.begin_action("rename").ok,"begin native delta action")
    check(doc.rename_mesh(mesh,"binding edit").ok and doc.get_document_summary().modified,"source edit reaches native session")
    check(doc.cancel_action().ok and not doc.get_document_summary().modified,"delta cancellation preserves native save baseline")
